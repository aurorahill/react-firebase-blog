...
const FirebaseService = {
  ...
  auth: {
    ...
    updateProfile: async (user, displayName, email) => {
      try {
  ...
  users: {
    ...
    fetchAllUserBlogs: async (userId) => {
      try {
    ...
    fetch6UserBlogs: async (userId, lastPaginationVisible = null) => {
      try {
        const blogRef = collection(db, "blogs");
    ...
    fetchNextUserBlogs: async (userId, lastPaginationVisible) => {
      try {
        const blogRef = collection(db, "blogs");
  ...
  blogs: {
    ...
    updateBlogComments: async (id, comments) => {
      try {
        const blogRef = doc(db, "blogs", id);
    ...
    updateBlogLikes: async (id, likes) => {
      try {
        const blogRef = doc(db, "blogs", id);
        const blogDoc = await getDoc(blogRef);
        if (!blogDoc.exists()) {
    ...
    update: async (blogId, updatedData, user) => {
      try {
